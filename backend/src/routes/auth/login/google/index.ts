...
const googleRoutes: FastifyPluginAsync = async (
    ...
    fastify.get('/callback', {
        schema: {
            ...
            response: {
                302: {
                    description: "Redirección exitosa con el token JWT o registro de usuario.",
                    type: 'object',
                    properties: {
                        token: { type: 'string' },
                        user: { type: 'object' }
        ...
        try {
            ...
            reply.redirect(`https://localhost/listado/verTodos/index.html?token=${token}&user=${user.id}`);
