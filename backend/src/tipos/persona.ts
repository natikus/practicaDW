...
export const PersonaSchema = Type.Object({
  ...
  nombre2: Type.Optional(
    Type.String({
      minLength: 2,
      maxLength: 50,
      description: "Segundo nombre de la persona si es que tiene",
    })
  ),
  ...
  email: Type.String({
    type: "string",
    format: "email",
    description: "Email relacionado a la persona",
  }),
  cedula: Type.String({
...
export const PersonaPutSchema = Type.Object({
  ...
  rut: Type.Optional(
    Type.String({ pattern: rutRegex.source, description: "RUT de la persona" })
  ),
  contrasena: Type.Optional(
    Type.String({ description: "Contraseña de la persona" })
  ),
});
